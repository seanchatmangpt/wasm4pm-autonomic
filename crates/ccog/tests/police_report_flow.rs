...
fn test_police_report_mvp() {
    ...
    field.graph.load_ntriples(TEST_FIELD_STATE)
        .expect("Failed to load field state");

    let verdict = process("The police report is missing", &mut field)
        .expect("process() failed");

    // Phrase bound to at least one public graph term
    assert!(!verdict.bound_terms.terms.is_empty(),
        "Expected bound terms from 'The police report is missing'");

    // Both tokens should bind
    assert!(verdict.bound_terms.terms.len() >= 1,
    ...
    assert!(!verdict.transition.admissible,
    ...
    let ask_result = field.graph.ask("ASK { ?s ?p ?o . FILTER(STRSTARTS(STR(?s), 'http://ccog')) }")
