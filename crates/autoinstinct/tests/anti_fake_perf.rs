...
fn performance_decide_does_not_materialize_or_seal() {
    ...
    for s in canonical_scenarios() {
        ...
        assert_eq!(
            bytes, 0,
            "scenario `{}`: decide() allocated under repeated calls —              likely materializing or sealing on the hot path",
            s.name
        );
    }
}
...
fn anti_fake_decide_is_zero_heap_and_input_dependent() {
    ...
    for s in canonical_scenarios() {
        ...
        for (pert, _expected, _) in &s.perturbations {
            ...
            assert!(
                changed,
                "scenario `{}` perturbation {:?}: response did not change                  (still {:?}) — input is not load-bearing",
                s.name, pert, after
            );
