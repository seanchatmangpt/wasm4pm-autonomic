...
fn gate_powl64cell_layout_exactly_64_bytes_aligned() {
    assert_eq!(
        size_of::<Powl64RouteCell>(),
        64,
        "Powl64RouteCell must be exactly 64 bytes to match proof spine laws."
    );
    assert_eq!(
        align_of::<Powl64RouteCell>(),
        64,
        "Powl64RouteCell must be 64-byte aligned."
    );
}
